...
typedef struct {
    ...
    // Constructor
    // Bullet(int16_t xPos, int16_t yPos, int16_t velX, int16_t velY) {
    //     x = xPos;
    //     y = yPos;
    //     vx = velX;
    //     vy = velY;
...
const uint16_t BulletImage[6 * 6] = {
  0xFFFF, 0xFFFF, 0xC618, 0xC618, 0xFFFF, 0xFFFF,   // Row 0 (top)
  0xFFFF, 0xC618, 0xC618, 0xA145, 0xA145, 0xFFFF,   // Row 1
  0xFFFF, 0xC618, 0xC618, 0xA145, 0xA145, 0xFFFF,   // Row 2
  0xFFFF, 0xFFFF, 0xA145, 0xA145, 0xFFFF, 0xFFFF,   // Row 3
  0xFFFF, 0xFFFF, 0xA145, 0xA145, 0xFFFF, 0xFFFF,   // Row 4
  0xFFFF, 0xFFFF, 0xFFFF, 0xFFFF, 0xFFFF, 0xFFFF    // Row 5 (bottom)
};
...
// Draws a 6x6 bullet at the given position with the specified color
static void drawBullet(int16_t x, int16_t y, uint16_t color);

// Spawns a bullet from the given tank and populates the projectile struct
void spawnBulletFromTank(Tank *playerOne, Bullet *bullet, bool playerTurn);

// Updates bullet position, checks for boundaries, applies gravity
void updateBullet(Bullet *bullet, Tank target, bool playerTurnBullet);
// Polls the button input and fires bullet if pressed
void pollButton(void);

// Converts ADC value to angle in degrees
static inline uint8_t PotAngle(uint32_t adc);

// Draws a line from (x0, y0) to (x1, y1) with the given color
void drawLine(int16_t x0, int16_t y0, int16_t x1, int16_t y1, uint16_t color);

// Updates the aiming line based on tank's angle and position
static inline void updateAimLine(void);
